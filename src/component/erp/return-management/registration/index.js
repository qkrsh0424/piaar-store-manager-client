...
const RegistrationComponent = (props) => {
    ...
    const __reqChangeCollectYnForReturnItemListSocket = async function (body) {
        await erpReturnItemSocket().changeCollectYnForList(body)
            .catch(err => {
                let res = err.response;
                if (res?.status === 500) {
                    alert('undefined error.');
    ...
    useEffect(() => {
        ...
        if(!defaultHeader || !defaultHeader.registrationHeaderId) {
            dispatchViewHeader({
                type: 'CLEAR'
            })
            return;
        ...
        let data = viewHeaderList.filter(r => r.id === defaultHeader.registrationHeaderId)[0];
        dispatchViewHeader({
            type: 'INIT_DATA',
            payload: data
        });
    ...
    // 수거중 처리 서밋
    const _onSubmit_changeCollectYnForReturnItemList = async (body) => {
        onActionOpenBackdrop();
        await __reqChangeCollectYnForReturnItemListSocket(body);
        onActionCloseBackdrop();
    }
    ...
    const _onSubmit_changeReturnReasonForReturnItemListInBatch = async function (body) {
        onActionOpenBackdrop()
