...
import ViewHeaderSettingModalComponent from './view-header-setting-modal-v4/ViewHeaderSettingModal.component';
...
const SalesComponent = (props) => {
    ...
    useEffect(() => {
        ...
        if(!query.headerId && viewHeader) {
            if (defaultHeader.salesHeaderId) {
                navigate({
                    pathname,
                    search: `?${qs.stringify({
                        ...query,
                        headerId: defaultHeader.salesHeaderId
                    })}`
                }, {
                    replace: true
                });
