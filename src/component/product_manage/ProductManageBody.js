        ...
        <>
            {userRdx.isLoading === false &&
                <Container className='mt-3'>
                    <div>
                        <AddBtn type='button'>출고 등록</AddBtn>
                        <button type='button'>입고 등록</button>
                    </div>
                    <TableContainer>
                        <table className="table table-sm" style={{ tableLayout: 'fixed' }}>
                            <thead>
                                <tr>
                                    ...
                                    <HeaderTh className='fixedHeader' scope="col" width='200'>상품명</HeaderTh>
                                        ...
                                        <React.Fragment key={productIdx}>
                                            <tr>
                                                ...
                                                <BodyTh style={{ textAlign: 'left' }} rowSpan={product.options.length + 1}>
                                                    ...
                                                    <div>M_Code : {product.product.manufaturingCode}</div>
                                                    <div>N_Code : {product.product.naverProductCode}</div>
                                                </BodyTh>
                                                <BodyTh rowSpan={product.options.length + 1}>
                                            ...
                                            {product.options && product.options.map((option, index2) => {
                                                return (
                                                    ...
                                                    // style={{ background: props.__handleEventControl().checkedOptionList().isChecked(option.id) ? '#7a7bda80' : 'none' }}
                                                    >
                                                        <BodyTd>
                                                            <input
                                                                type='checkbox'
