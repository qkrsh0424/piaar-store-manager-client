import { Container, GraphFieldWrapper } from "./ProductBestOptionGraph.styled";
...
const DEFAULT_GRAPH_BG_3COLOR = ['#FFBCA2', '#FFB2BA'];

const BEST_UNIT = 15;
// 판매스토어별 총 매출액
export default function ProductBestOptionGraphComponent(props) {
    ...
    }, [props.selectedProduct, props.performance])
    ...
    const __handle = {
        action: {
            ...
            createPayAmountGraphData: () => {
                let selectedProductPerformance = props.performance?.filter(r => props.selectedProduct.includes(r.productDefaultName));
                let bestSalesPayAmount = _.sortBy(selectedProductPerformance, 'salesPayAmount').reverse().slice(0, BEST_UNIT);
            ...
            createUnitDate: () => {
                let selectedProductPerformance = props.performance?.filter(r => props.selectedProduct.includes(r.productDefaultName));
                let bestSalesUnit = _.sortBy(selectedProductPerformance, 'salesUnit').reverse().slice(0, BEST_UNIT);
