    ...
    const returnConvertedOptions = () => {
        ...
        try {
            ...
            for (var i = 0; i < defaultNameList.length; i++) {
                ...
                if (salesPriceList[i] && !isNumberFormat(salesPriceList[i])) {
                    throw Error('판매가에 숫자만 입력해주세요.');
                }

                if (totalPurchasePriceList[i] && !isNumberFormat(totalPurchasePriceList[i])) {
                    throw Error('매입총합계에 숫자만 입력해주세요.');
                }

                if (safetyStockUnitList[i] && !isNumberFormat(safetyStockUnitList[i])) {
